...
export async function callTool(env, projectId, toolName, args) {
    try {
        switch (toolName) {
            case "plan_task": {
                ...
                for (const session of activeSessions) {
                    if (session.github_username !== owner) {
                        const overlap = session.file_paths.filter((fp) => target_files.some((tf) => fp.includes(tf.replace(/\*/g, ""))));
                        if (overlap.length > 0) {
                ...
                for (const node of existingNodes) {
                    if (node.status === "in_progress" && node.owner_github_username !== owner) {
                        const overlap = node.target_paths.filter((tp) => target_files.some((tf) => tp.includes(tf.replace(/\*/g, "")) || tf.includes(tp.replace(/\*/g, ""))));
                        if (overlap.length > 0) {
            ...
            case "check_collision": {
                ...
                for (const session of activeSessions) {
                    const overlap = session.file_paths.filter((fp) => file_paths.some((f) => f.includes(fp) || fp.includes(f)));
                    if (overlap.length > 0) {
                ...
                const commitCollisions = [];
                for (const commit of recentCommits) {
                    const overlap = commit.file_paths.filter((fp) => file_paths.some((f) => f.includes(fp) || fp.includes(f)));
                    if (overlap.length > 0) {
            ...
            case "get_recent_decisions": {
                const { topic } = args;
                const query = { project_id: `eq.${projectId}` };
                let decisions = await db.select(env, "decisions", query);
                if (topic) {
                    decisions = decisions.filter(d => d.topic.toLowerCase().includes(topic.toLowerCase()) ||
            ...
            case "check_semantic_duplicate": {
                ...
                const matchLines = matches.map((m) => `- \`${m.module_name}.${m.function_name}${m.signature}\` in [${m.file_path}](file:///${m.file_path}) (Similarity: ${(m.similarity * 100).toFixed(1)}%)`);
                return {
                    content: [{
                            type: "text",
