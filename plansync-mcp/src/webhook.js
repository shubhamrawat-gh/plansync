...
async function fetchGitHubFile(env, repoFullName, path, ref) {
...
async function commitGitHubFile(env, repoFullName, path, content, sha, message, branch) {
    const token = env.GITHUB_TOKEN;
    if (!token)
        return false;
...
export async function handleWebhook(request, env) {
    ...
    for (const commit of payload.commits || []) {
        ...
        const isDecision = commit.message.toLowerCase().includes("decided") ||
        ...
        for (const node of planNodes) {
            const match = filePaths.some((fp) => node.target_paths.some((tp) => {
            ...
            if (match) {
                ...
                if (newStatus !== node.status) {
                    await db.update(env, "plan_nodes", { id: `eq.${node.id}` }, { status: newStatus, updated_at: new Date().toISOString() });
                }
            }
        }
...
async function syncAgentsMarkdown(env, projectId, repoFullName, branch) {
    const token = env.GITHUB_TOKEN;
    if (!token)
        return;
...
`;
    ...
    if (updatedContent !== agentsMdContent) {
        await commitGitHubFile(env, repoFullName, "AGENTS.md", updatedContent, fileSha, "docs: sync AGENTS.md auto-generated sections [skip ci]", branch);
    }
}
function replaceSection(content, sectionName, sectionBody) {
